    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-black/90 p-3 rounded-lg border border-indigo-500/30 backdrop-blur-sm text-sm">
          ...
          <p className="text-indigo-300">{`Revenue: ${formatCurrency(
            payload[0].payload.revenue
          )}`}</p>
          <p className="text-red-300">{`Costs: ${formatCurrency(
            payload[0].payload.costs
          )}`}</p>
          <p
            className={`font-medium ${
              payload[0].payload.profit >= 0 ? "text-green-300" : "text-red-300"
            }`}
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Adjust the sliders to simulate different business scenarios and see
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-indigo-900/20 rounded-lg p-4 border border-indigo-500/20">
                ...
                <div
                  className={`text-xl font-bold ${
                    monthlyProfit >= 0 ? "text-white" : "text-red-400"
                  }`}
                >
              ...
              {monthlyProfit >= 0 ? (
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-300 text-sm mt-1">
                      At {unitsSold} units sold monthly (with a selling price of
                      ${sellingPrice}), your business generates $
                      {monthlyProfit.toFixed(0)} in monthly profit. This is{" "}
                      {Math.abs(unitsSold - breakEvenUnits)}{" "}
                      {unitsSold > breakEvenUnits
                        ? "units above"
                        : "units below"}{" "}
                      the break-even point of {breakEvenUnits} units.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-300 text-sm mt-1">
                      At {unitsSold} units sold monthly (with a selling price of
                      ${sellingPrice}), your business loses $
                      {Math.abs(monthlyProfit).toFixed(0)} monthly. You need to
                      sell {Math.abs(unitsSold - breakEvenUnits)} more units to
                      reach the break-even point of {breakEvenUnits} units.
                    </p>
                  </div>
                </div>
              )}
