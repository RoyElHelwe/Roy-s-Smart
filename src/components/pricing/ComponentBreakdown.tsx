  ...
  const components: ComponentCost[] = [
    {
      ...
      icon: <Battery />,
      color: "bg-blue-500",
      description: "High-capacity, long-lasting lithium battery pack provides reliable power for all your devices. Engineered with premium cells for maximum longevity and performance."
    },
    {
      id: "inverter",
      ...
      icon: <Zap />,
      color: "bg-yellow-500",
      description: "Pure sine wave inverter delivers clean AC power for laptops and other sensitive electronics. Efficient operation with low heat generation and multiple protection circuits."
    },
    {
      id: "controller",
      ...
      icon: <Cpu />,
      color: "bg-green-500",
      description: "Smart power management system optimizes charging and power distribution. Features dynamic load balancing and protection circuits for battery longevity."
    },
    {
      id: "ports",
      name: "Ports & Display",
      ...
      icon: <Cable />,
      color: "bg-purple-500",
      description: "Comprehensive connection panel includes USB-C, USB-A, AC outlets, and an intuitive display for monitoring power status. High-quality components ensure reliable connections."
    },
    {
      id: "frame",
      ...
      icon: <Layout />,
      color: "bg-indigo-500",
      description: "Aerospace-grade aluminum frame with precision hinges and adjustable components. Engineered for durability while maintaining a lightweight profile."
    },
    {
      id: "cable",
      ...
      icon: <Cable />,
      color: "bg-pink-500",
      description: "Integrated cable management solution keeps your workspace organized and free from tangled wires. Includes retractable cable guides and storage compartments."
    },
    {
      id: "cooling",
      name: "Cooling System",
      ...
      icon: <Zap />,
      color: "bg-teal-500",
      description: "Advanced thermal management system ensures optimal performance in various environments. Features passive cooling design with strategic heat dissipation."
    },
    {
      id: "indicators",
      name: "LED Indicators",
      ...
      icon: <Zap />,
      color: "bg-amber-500",
      description: "Intuitive status indicators provide at-a-glance information about power levels, charging status, and connectivity. Energy-efficient LEDs with adjustable brightness."
    },
    {
      id: "modem",
      ...
      icon: <Wifi />,
      color: "bg-red-500",
      description: "High-gain cellular modem with advanced antenna design ensures connectivity even in areas with weak signal. Multi-band support for global compatibility."
    },
    {
      id: "assembly",
      ...
      icon: <Shield />,
      color: "bg-gray-500",
      description: "Each unit is carefully assembled and undergoes rigorous quality testing to ensure reliability and performance. Includes burn-in testing and calibration."
    }
  ];
    ...
    return (
      <div className="relative">
        <svg width="200" height="200" viewBox="0 0 200 200">
          {pieSlices.map((slice, index) => (
            <motion.path
              ...
              style={{ 
                opacity: hoveredPieSlice === null || hoveredPieSlice === slice.id ? 1 : 0.3,
        ...
        {hoveredPieSlice && (
          ...
          >
            {components.find(c => c.id === hoveredPieSlice)?.name}: ${components.find(c => c.id === hoveredPieSlice)?.cost}
          </motion.div>
        )}
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {components.map((component, index) => (
                <motion.div
                  ...
                  className={`rounded-lg overflow-hidden border border-gray-800 transition-all duration-300 ${
                    hoveredPieSlice === component.id || expandedComponent === component.id
                      ? `bg-black/60 border-${component.color.replace('bg-', '')}/30`
                      : 'bg-black/30'
                  }`}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: isVisible ? 1 : 0, y: isVisible ? 0 : 20 }}
                ...
                >
                  ...
                  >
                    <div className="flex items-center">
                      <div className={`p-2 rounded-full ${component.color} mr-3`}>
                        {component.icon}
                      </div>
                      <div>
                        <div className="font-medium text-white">{component.name}</div>
                        <div className="text-sm text-gray-400">${component.cost} ({getPercentage(component.cost)}%)</div>
                      </div>
                    </div>
                    <ChevronDown 
                      className={`h-5 w-5 text-gray-400 transition-transform duration-300 ${
                        expandedComponent === component.id ? 'transform rotate-180' : ''
                      }`} 
                    />
                  </div>
                  ...
                  {expandedComponent === component.id && (
                    ...
                    >
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-800 flex justify-between text-xs text-gray-400">
                        <div>
                          Unit Cost: <span className="text-white">${component.cost}</span>
                        </div>
                        <div>
                          Percentage: <span className="text-white">{getPercentage(component.cost)}%</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
        ...
        >
          ...
          <p className="text-gray-300">
            At Roy's Smart, we believe in using only the highest-quality components to ensure reliability, durability, and performance. We source premium materials and components directly from reputable manufacturers to maintain strict quality control while providing exceptional value.
          </p>
